...
g <- ggplot(resultado_final, aes(x = JJM2017, y = n_cuadros, fill = escala)) +
  geom_col() +
  labs(x = "Biogeographic province", y = "Number of cells with records") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
#Save it
ggsave("data/out/scale_analysis/scale_analysis_english.png",g, 
...
# Ejemplo con una de tus escalas, puedes iterar después
grid_shapefile <- "data/out/sf_prov_grid/grid_0.315.shp"
puntos_csv <- "data/in/hgbif_completo_iucn.csv"

# Intersección de puntos y grid
intersect <- intersect_points_with_grid(grid_shapefile, puntos_csv)

# Filtrar registros válidos (sin NA en el nombre de la especie)
registros_validos <- intersect %>%
  filter(!is.na(correctname)) %>%
  as.data.table()

# Calcular riqueza por celda
tabla_riqueza <- registros_validos[, .(
  region_biogeo = unique(JJM2017),
  riqueza = uniqueN(correctname, na.rm = TRUE)
), by = grid_id]
