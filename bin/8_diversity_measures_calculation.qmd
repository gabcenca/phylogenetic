...
ras <- rast(here::here("data/out/raster_species/species_tree/ras_species_res_0.5.tif"))


# Standard analysis

#tabla con especies ordenadas
dataprep <- phylo.pres(x = sub_dist[[3]], tree = sub_tree)
...
pd_ses_sim5 <- rast.pd.ses(x = dataprep$x, 
                            edge.path = dataprep$edge.path,
                          branch.length = dataprep$branch.lenght,
                          aleats = 10,
                          random = "tip") 
...
pd_ses_sim9 <- rast.pd.ses(x = species_raster,
                            tree = oak_tree,
                            aleats = 999,
                            spat_alg = "bootspat_ff")   # SIM9

# Compare results
# If both show similar patterns → strong evidence
# If only SIM5 significant → moderate evidence (depends on preserved spatial structure)
# If neither significant → patterns don't differ from null expectations
```
