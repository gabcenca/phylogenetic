...
# Spit the state area into a grid
mex_grid <- mexico_map_ITRF %>%
  st_make_grid(cellsize = c(100000, 100000))  # Grid with 75 km x 75 km cells
...
richness_per_grid_box <- mex_grid_map %>%
  ...
  group_by(grid_id) %>%
  summarize(total = sum(!is.na(distinct(correctname))


#Set value of 0 records to NA so we can graph it in different color 
richness_per_grid_box$total[richness_per_grid_box$total == 0] <- NA
