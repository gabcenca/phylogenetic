...
gbif_herb_raw_all_observations <- bind_rows(herb_corr_raw, gbif_corr_raw)

```

-Then filter the records that have only genera and the records that are reliable
```{r}
gbif_herb_raw <- gbif_herb_raw_all_observations %>% filter(!scientificName %in% c("Quercus","Quercus L.")) %>%
  filter(!basisOfRecord %in% c("MATERIAL_SAMPLE", "MATERIAL_CITATION" ,"FOSSIL_SPECIMEN"))
```
